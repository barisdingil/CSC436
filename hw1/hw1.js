...
const Product = class Product {
    constructor(oneString){
        this.name = oneString.split(',')[0];
        this.price = Math.floor(oneString.split(',')[1]) + (oneString.split(',')[1]%1);
        this.availability = oneString.split(',')[2];
    ...
    static inStock = (products) => { 
        return products.filter(inStock => inStock.availability.indexOf("In Stock") !== -1);
    }
    ...
    static halfOff = (products) => {
        return products.map(product =>{return{...product,price:product.price/2}});
    }
    ...
    static printProducts = (products) => {
        products.forEach(prod => console.log("Product: "+prod.name+", Cost: "+(formatter.format(prod.price)+", Availability: "+(prod.availability=== "In Stock" ? "Yes" : "No"))));
    }
 };
